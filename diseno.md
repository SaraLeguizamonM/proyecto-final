...
La lógica de descubrirCascada fue uno de los apartados que más tiempo llevó. Para verificar los ocho vecinos de cada celda usamos un arreglo con los desplazamientos de +1 y -1 en fila y columna (valores "quemados"). En lugar de recursividad, usamos un ArrayList como pila manual, cuando se descubre una celda vacía se agrega a la lista, y un ciclo while procesa cada pendiente hasta que no queden más. Tuvimos cuidado en que el ciclo se detenga al encontrar celdas ya descubiertas, con bandera o con minas adyacentes, evitando bucles infinitos u otros errores.
