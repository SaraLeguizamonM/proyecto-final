    ...
    public void setDimensiones(int filas, int columnas, int minas) {
        ...
        if (filas < 3 || filas > 30) {
            throw new IllegalArgumentException(
                    "El valor de las filas no puede ser menor a 3 o mayor a 30, reingrese nuevamente un valor válido entre 3 y 30.");
        }
        this.filas = filas;

        // Validamos el valor de las columnas antes de asignarlas, y se lanza la
        // excepcion
        // en caso de no cumplir con la restriccion
        if (columnas < 3 || columnas > 30) {
            throw new IllegalArgumentException(
                    "El valor de las columnas no puede ser menor a 3 o mayor a 30, reingrese nuevamente un valor válido entre 3 y 30.");
        }
        ...
        if (minas <= 0 || minas > maximoMinasPermitido) {
            throw new IllegalArgumentException(
                    "La cantidad de minas debe ser mayor a 0 y no puede superar la mitad del tablero, que es("
                            + maximoMinasPermitido + "), reingrese nuevamente el valor.");
        }
        this.minas = minas;
    }
}